...
- The keymap layer is registered via the v2.0.8 blessed pattern: the slot render returns a `Commands` component element (`append: "app"`, `src/commands.tsx`) and the component body calls `keymap.layer()`. A bare render callback that calls `layer()` itself fails silently — the keymap hooks need a component owner, so the layer never registers despite setup completing. `keymap.dispatch` and `keymap.mode.current` remain guarded at call time.
